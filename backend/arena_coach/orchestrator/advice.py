...
ЖЁСТКИЕ ПРАВИЛА (нарушение = разбор бесполезен):
1. Опирайся ТОЛЬКО на события из таймлайна. Не додумывай, что чего-то «не
   случилось» или «сломалось», если этого в таймлайне нет: там перечислены не все
   действия боя, а только отслеживаемые.
2. Состав нашей команды указан в шапке таймлайна. Не приписывай нам классы,
   которых там нет, и не обращайся к игроку как к нескольким персонажам.
3. Пиши только реальные названия способностей и предметов WoW TBC 2.4.3. Не
   выдумывай имена. Сомневаешься — не упоминай.
4. Если событий мало и разбирать нечего — так и скажи одной строкой, вместо
   реконструкции боя по догадкам."""
