...
Опенер: наш рог ищет их рога (care: их hpala может выбить нашего рога из стелса Consecration'ом — это прямой совет их гайда; держись вне 8-ярдовой зоны паладина). Маг открывает шип на паладина ТОЛЬКО как бейт (Cleanse снимет — но потратит его GCD/ману), реальный сетап: чип→кидни на их роге + нова-шаттер, кс на heal-школу паладина в килл-окно. Burst вкладываем, когда у паладина нет freedom/BoP/bubble — Hesback прямо описывает их экономию: BoP держат на 2-й кидни, трикет — на 2-й блайнд. Значит, первый блайнд на паладина обычно проходит → используем его для чистого кила-окна по рогу.

## Alternative опенер

Если их рог открылся первым на маге (чип→кидни, сверху подж паладина): маг трикетит kidney (не их блайнд), блок под ярость, наш рог немедленно перекрывает их рога кидни/Gouge и снимает давление. Дальше — назад к нашему сетапу: их hpala после раннего HoJ остаётся без стана на ~минуту.

## If enemy trinkets

Их пал по гайду Hesback **не** трикетит первый сетап — держит трикет под наш блайнд (BoP-очередь: 2-й kidney). Если паладин всё же трикетнул рано — это окно: блайнд залипнет, маг свободно кастует в их рога. Их рог трикетит кидни/шип; после его трикета — полный шаттер-сетап. Dwarf-паладин: Stoneform = вторая freedom + сброс wound (Hesback) — не считай его законтроленным по снарам.
