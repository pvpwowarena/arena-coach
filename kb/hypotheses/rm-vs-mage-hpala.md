---
slug: rm-vs-mage-hpala
schema_version: 1
expansion: tbc
composition: rogue+mage
vs: mage+holy-paladin
bracket: 2v2
difficulty: hard
kill_target:
  primary: mage
  fallback: paladin
provenance: "AI-synthesized (Claude), UNVERIFIED — нет внешнего источника"
confidence: hypothesis
status: sourced-promoted
promoted_to: kb/drafts/rm-vs-mage-hpala.md
promoted_at: '2026-07-07'
tags: [ai-synthesized, unverified, needs-source-or-review]
---

> ✅ **ЗАСОРСЕНО (2026-07-07).** Promoted to `kb/drafts/rm-vs-mage-hpala.md`. Якорь пары: Skill Capped (Season 2, Patch 2.5.5, upd. 2026-01-01) **называет пару тиром** на двух страницах — hpala-comps и fmage-comps: «C Tier: Holy Paladin + Frost Mage» (наш RM там же — S Tier). Это ровно условие (а) из пере-проверки 06-30: источник, оценивающий комбо. Исполнение синтезировано (тег `synthesized-execution`), difficulty в драфте C-tier-якорем снижена hard → moderate. Awaits owner approval via `python -m arena_ingest review approve --slug rm-vs-mage-hpala`.
>
> ⚠ **AI-СИНТЕЗ, НЕ ПРОВЕРЕНО.** Из общих знаний по TBC 2.4.3, без гайда/стрима. Не отдавать игрокам как факт из базы. Нужен реальный источник или ревью топ-игрока перед переносом в kb/drafts/.

## Opener

Frost-маг + holy paladin: вражеский маг (burst/контроль) под защитой пала-хила (bubble/freedom/cleanse/[[ability:hammer-of-justice]]/sacred shield). [[ability:sheep]] снимается cleanse'ом, freedom снимает [[ability:nova]]-root.

Kill target — вражеский маг (сквишовее, и убрав его — 2v1 на пала, который не убьёт). Опенер: рог [[ability:sap]] паладина, рог+маг на вражеском маге. Наш маг [[ability:counterspell]] держит на heal паладина ИЛИ на каст вражеского мага (фейк-бейт), рог [[ability:kidney-shot]]/[[ability:blind]] не даёт магу блинкать-кайтить. Burst (shatter + [[ability:kidney-shot]]) в окно без freedom/bubble.

## Alternative opener

Если открыли на тебе: вражеский [[ability:nova]]+[[ability:sheep]], [[ability:hammer-of-justice]] паладина. [[ability:ice-block]] под burst, рог [[ability:cheap-shot]]/[[ability:kidney-shot]] разрывает их сетап, [[ability:vanish]] из их CC и заход заново на мага.

## If enemy trinkets

Paladin трикетит [[ability:sheep]]/[[ability:hammer-of-justice]]; маг трикетит [[ability:kidney-shot]]/[[ability:nova]]. Держи [[ability:counterspell]] на heal паладина; не бурсти при доступных bubble/freedom.

## Common mistakes

- Бурстить вражеского мага в bubble/freedom-окне.
- [[ability:sheep]]'ить паладина (cleanse снимет) без бейта КД.
- Дать вражескому магу блинкать на дистанцию — рог обязан держать [[ability:kidney-shot]]/[[ability:blind]].

## Key cooldowns to track

- enemy: mage — [[ability:ice-block]], blink, [[ability:counterspell]], [[ability:sheep]], [[ability:nova]], trinket; paladin — bubble, freedom, cleanse, [[ability:hammer-of-justice]], trinket.
- ours: [[ability:counterspell]], [[ability:sheep]], [[ability:nova]], [[ability:blind]], [[ability:kidney-shot]], [[ability:vanish]], [[ability:ice-block]], trinket.
