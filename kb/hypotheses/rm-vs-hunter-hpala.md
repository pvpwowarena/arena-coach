---
slug: rm-vs-hunter-hpala
schema_version: 1
expansion: tbc
composition: rogue+mage
vs: hunter+holy-paladin
bracket: 2v2
difficulty: hard
kill_target:
  primary: hunter
  fallback: paladin
provenance: "AI-synthesized (Claude), UNVERIFIED — нет внешнего источника"
confidence: hypothesis
tags: [ai-synthesized, unverified, needs-source-or-review]
---

> ⚠ **AI-СИНТЕЗ, НЕ ПРОВЕРЕНО.** Из общих знаний по TBC 2.4.3, без гайда/стрима. Не отдавать игрокам как факт из базы. Нужен реальный источник или ревью топ-игрока перед переносом в kb/drafts/.

## Opener

Hunter + holy paladin: дальнее давление + ловушки/контроль от хантера, пала держит хантера хилом. Угрозы хантера: freezing trap, [[ability:scatter-shot]], viper sting (мана — бьёт по магу), wing clip, deterrence (survival), pet, intimidation (BM). Пала — bubble/freedom/cleanse/[[ability:hammer-of-justice]]/sacred shield. [[ability:sheep]] снимается cleanse'ом.

План: kill target — хантер (пала почти неубиваем). Рог открывает на хантере, не давая ему дистанцию: [[ability:cheap-shot]]/[[ability:kidney-shot]], рвём kite. Маг [[ability:counterspell]] держит на heal паладина, [[ability:nova]] для shatter-окна по хантеру. Берегись trap/scatter на входе (стой раздельно, чтобы не словить trap по обоим). Можно валить pet, чтобы снять часть давления.

## Alternative opener

Если открыли на тебе: trap/[[ability:scatter-shot]] на роге, viper на маге, [[ability:hammer-of-justice]]. [[ability:ice-block]] под burst/трап, рог уходит [[ability:vanish]] из trap-сетапа и заходит заново на хантера.

## If enemy trinkets

Paladin тринкетит [[ability:sheep]]/[[ability:hammer-of-justice]]; хантер тринкетит [[ability:kidney-shot]]/[[ability:sheep]] или [[ability:nova]]. Держи [[ability:counterspell]] на heal; не входи в открытую на freezing trap.

## Common mistakes

- Стоять кучно — freezing trap + AoE накроют обоих.
- Гнаться за палой; [[ability:sheep]]'ить хилера (cleanse снимет).
- Не закрывать viper sting на маге — маг уходит в OOM, теряем бурст.

## Key cooldowns to track

- enemy: hunter — freezing trap, [[ability:scatter-shot]], viper sting, deterrence, pet, intimidation, trinket; paladin — bubble, freedom, cleanse, [[ability:hammer-of-justice]], trinket.
- ours: [[ability:counterspell]], [[ability:nova]], [[ability:sheep]], [[ability:blind]], [[ability:kidney-shot]], [[ability:vanish]], [[ability:ice-block]], trinket.
