---
slug: rm-vs-hunter-rdruid
schema_version: 1
expansion: tbc
composition: rogue+mage
vs: hunter+resto-druid
bracket: 2v2
difficulty: easy
kill_target:
  primary: hunter
maps_notes: {}
sources:
- type: file
  path: WOW TBC ARENA - Rogue  Mage.md
  lines: 158-178
  author: Mirlol (transcribed)
  retrieved: '2026-05-12'
last_reviewed: '2026-05-12'
reviewer: null
confidence: draft
tags: []
---

## Opener

Маг идёт в invis и сообщает где hunter (если по какой-то причине он не сидит на flare'е). Когда маг выходит из invis'а, он поднимает water elemental и соло-бьёт hunter'а через nova'ы, cone of cold'ы, veins, блокируя/тринкетя любые CC; держит hunter'а в slow'е.

Пока маг соло'ит hunter'а, поднимай goggles и/или perception, ищи друида и стань его сразу как нашёл. После cheap shot'а — [[ability:gouge]] → [[ability:kidney]] и беги к hunter'у добивать. Маг должен [[ability:counterspell]] друида после kidney.

Если друид тринкетит kidney — [[ability:blind]] его и продолжаешь убивать hunter'а. Я обычно держу double wound даже если hunter dwarf — потому что маг и так slow'ит hunter'а.

Часто друид сидит прямо на flare'е рядом с hunter'ом, иногда в tree form'е с pre-hot'ами. Но стратегия обычно не меняется.

Бывает что маг не вытягивает соло против hunter'а из-за неудачного RNG. В этом случае после неудачной попытки убийства можно sheep'нуть hunter'а и попробовать убить друида вместо него.
