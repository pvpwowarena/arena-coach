# Матрица покрытия матчапов (auto-generated)

`python tools/coverage_matrix.py` — перегенерировать. Уровень классов; ✅ sourced draft · 🟡 AI-hypothesis (kb/hypotheses/, непроверено) · ⬜ todo.
**Итого (класс-уровень):** ✅ 49 sourced · 🟡 4 hypotheses · ⬜ 202 todo (всего ячеек 255).

**Спек-варианты (сверх 255):** ✅ 2 · 🟡 0 · ⬜ 0.


## 2v2 — 45 вражеских комбинаций × 2 наших

| Enemy \ Our | RM | RP |
|---|---|---|
| warrior+warrior | ⬜ | ⬜ |
| warrior+paladin | ✅ | ✅ |
| warrior+hunter | ⬜ | ⬜ |
| warrior+rogue | ✅ | ✅ |
| warrior+priest | ⬜ | ⬜ |
| warrior+shaman | ✅ | ✅ |
| warrior+mage | ✅ | ✅ |
| warrior+warlock | ⬜ | ⬜ |
| warrior+druid | ✅ | ✅ |
| paladin+paladin | ⬜ | ⬜ |
| paladin+hunter | 🟡 | ✅ |
| paladin+rogue | ✅ | ✅ |
| paladin+priest | ⬜ | ⬜ |
| paladin+shaman | ✅ | ✅ |
| paladin+mage | ✅ | ✅ |
| paladin+warlock | ✅ | ✅ |
| paladin+druid | ⬜ | ⬜ |
| hunter+hunter | ⬜ | ⬜ |
| hunter+rogue | ⬜ | ⬜ |
| hunter+priest | ⬜ | ⬜ |
| hunter+shaman | 🟡 | 🟡 |
| hunter+mage | ⬜ | ⬜ |
| hunter+warlock | ⬜ | ⬜ |
| hunter+druid | ✅ | ✅ |
| rogue+rogue | ✅ | ✅ |
| rogue+priest | ✅ | ✅ |
| rogue+shaman | ⬜ | ⬜ |
| rogue+mage | ✅ | ✅ |
| rogue+warlock | ✅ | ✅ |
| rogue+druid | ✅ | ✅ |
| priest+priest | ⬜ | ⬜ |
| priest+shaman | ⬜ | ⬜ |
| priest+mage | ✅ | ✅ |
| priest+warlock | ✅ | ✅ |
| priest+druid | ⬜ | ⬜ |
| shaman+shaman | ⬜ | ⬜ |
| shaman+mage | ⬜ | ⬜ |
| shaman+warlock | ✅ | ✅ |
| shaman+druid | ⬜ | ⬜ |
| mage+mage | ⬜ | ⬜ |
| mage+warlock | ⬜ | ⬜ |
| mage+druid | 🟡 | ✅ |
| warlock+warlock | ⬜ | ⬜ |
| warlock+druid | ✅ | ✅ |
| druid+druid | ⬜ | ⬜ |

## 3v3 — 165 вражеских комбинаций × 1 наших

| Enemy \ Our | RMP |
|---|---|
| warrior+warrior+warrior | ⬜ |
| warrior+warrior+paladin | ⬜ |
| warrior+warrior+hunter | ⬜ |
| warrior+warrior+rogue | ⬜ |
| warrior+warrior+priest | ⬜ |
| warrior+warrior+shaman | ⬜ |
| warrior+warrior+mage | ⬜ |
| warrior+warrior+warlock | ⬜ |
| warrior+warrior+druid | ⬜ |
| warrior+paladin+paladin | ⬜ |
| warrior+paladin+hunter | ⬜ |
| warrior+paladin+rogue | ⬜ |
| warrior+paladin+priest | ⬜ |
| warrior+paladin+shaman | ⬜ |
| warrior+paladin+mage | ⬜ |
| warrior+paladin+warlock | ⬜ |
| warrior+paladin+druid | ⬜ |
| warrior+hunter+hunter | ⬜ |
| warrior+hunter+rogue | ⬜ |
| warrior+hunter+priest | ⬜ |
| warrior+hunter+shaman | ⬜ |
| warrior+hunter+mage | ⬜ |
| warrior+hunter+warlock | ⬜ |
| warrior+hunter+druid | ⬜ |
| warrior+rogue+rogue | ⬜ |
| warrior+rogue+priest | ⬜ |
| warrior+rogue+shaman | ⬜ |
| warrior+rogue+mage | ⬜ |
| warrior+rogue+warlock | ⬜ |
| warrior+rogue+druid | ⬜ |
| warrior+priest+priest | ⬜ |
| warrior+priest+shaman | ⬜ |
| warrior+priest+mage | ✅ |
| warrior+priest+warlock | ⬜ |
| warrior+priest+druid | ⬜ |
| warrior+shaman+shaman | ⬜ |
| warrior+shaman+mage | ⬜ |
| warrior+shaman+warlock | ⬜ |
| warrior+shaman+druid | ⬜ |
| warrior+mage+mage | ⬜ |
| warrior+mage+warlock | ⬜ |
| warrior+mage+druid | ⬜ |
| warrior+warlock+warlock | ⬜ |
| warrior+warlock+druid | ✅ |
| warrior+druid+druid | ⬜ |
| paladin+paladin+paladin | ⬜ |
| paladin+paladin+hunter | ⬜ |
| paladin+paladin+rogue | ⬜ |
| paladin+paladin+priest | ⬜ |
| paladin+paladin+shaman | ⬜ |
| paladin+paladin+mage | ⬜ |
| paladin+paladin+warlock | ⬜ |
| paladin+paladin+druid | ⬜ |
| paladin+hunter+hunter | ⬜ |
| paladin+hunter+rogue | ⬜ |
| paladin+hunter+priest | ⬜ |
| paladin+hunter+shaman | ⬜ |
| paladin+hunter+mage | ⬜ |
| paladin+hunter+warlock | ⬜ |
| paladin+hunter+druid | ⬜ |
| paladin+rogue+rogue | ⬜ |
| paladin+rogue+priest | ⬜ |
| paladin+rogue+shaman | ⬜ |
| paladin+rogue+mage | ⬜ |
| paladin+rogue+warlock | ⬜ |
| paladin+rogue+druid | ⬜ |
| paladin+priest+priest | ⬜ |
| paladin+priest+shaman | ⬜ |
| paladin+priest+mage | ⬜ |
| paladin+priest+warlock | ⬜ |
| paladin+priest+druid | ⬜ |
| paladin+shaman+shaman | ⬜ |
| paladin+shaman+mage | ⬜ |
| paladin+shaman+warlock | ⬜ |
| paladin+shaman+druid | ⬜ |
| paladin+mage+mage | ⬜ |
| paladin+mage+warlock | ⬜ |
| paladin+mage+druid | ⬜ |
| paladin+warlock+warlock | ⬜ |
| paladin+warlock+druid | ⬜ |
| paladin+druid+druid | ⬜ |
| hunter+hunter+hunter | ⬜ |
| hunter+hunter+rogue | ⬜ |
| hunter+hunter+priest | ⬜ |
| hunter+hunter+shaman | ⬜ |
| hunter+hunter+mage | ⬜ |
| hunter+hunter+warlock | ⬜ |
| hunter+hunter+druid | ⬜ |
| hunter+rogue+rogue | ⬜ |
| hunter+rogue+priest | ⬜ |
| hunter+rogue+shaman | ⬜ |
| hunter+rogue+mage | ⬜ |
| hunter+rogue+warlock | ⬜ |
| hunter+rogue+druid | ⬜ |
| hunter+priest+priest | ⬜ |
| hunter+priest+shaman | ⬜ |
| hunter+priest+mage | ⬜ |
| hunter+priest+warlock | ⬜ |
| hunter+priest+druid | ✅ |
| hunter+shaman+shaman | ⬜ |
| hunter+shaman+mage | ⬜ |
| hunter+shaman+warlock | ⬜ |
| hunter+shaman+druid | ⬜ |
| hunter+mage+mage | ⬜ |
| hunter+mage+warlock | ⬜ |
| hunter+mage+druid | ⬜ |
| hunter+warlock+warlock | ⬜ |
| hunter+warlock+druid | ⬜ |
| hunter+druid+druid | ⬜ |
| rogue+rogue+rogue | ⬜ |
| rogue+rogue+priest | ⬜ |
| rogue+rogue+shaman | ⬜ |
| rogue+rogue+mage | ⬜ |
| rogue+rogue+warlock | ⬜ |
| rogue+rogue+druid | ⬜ |
| rogue+priest+priest | ⬜ |
| rogue+priest+shaman | ⬜ |
| rogue+priest+mage | ✅ |
| rogue+priest+warlock | ✅ |
| rogue+priest+druid | ⬜ |
| rogue+shaman+shaman | ⬜ |
| rogue+shaman+mage | ⬜ |
| rogue+shaman+warlock | ⬜ |
| rogue+shaman+druid | ⬜ |
| rogue+mage+mage | ⬜ |
| rogue+mage+warlock | ⬜ |
| rogue+mage+druid | ⬜ |
| rogue+warlock+warlock | ⬜ |
| rogue+warlock+druid | ✅ |
| rogue+druid+druid | ⬜ |
| priest+priest+priest | ⬜ |
| priest+priest+shaman | ⬜ |
| priest+priest+mage | ⬜ |
| priest+priest+warlock | ⬜ |
| priest+priest+druid | ⬜ |
| priest+shaman+shaman | ⬜ |
| priest+shaman+mage | ✅ |
| priest+shaman+warlock | ✅ |
| priest+shaman+druid | ⬜ |
| priest+mage+mage | ⬜ |
| priest+mage+warlock | ✅ |
| priest+mage+druid | ⬜ |
| priest+warlock+warlock | ⬜ |
| priest+warlock+druid | ⬜ |
| priest+druid+druid | ⬜ |
| shaman+shaman+shaman | ⬜ |
| shaman+shaman+mage | ⬜ |
| shaman+shaman+warlock | ⬜ |
| shaman+shaman+druid | ⬜ |
| shaman+mage+mage | ⬜ |
| shaman+mage+warlock | ⬜ |
| shaman+mage+druid | ⬜ |
| shaman+warlock+warlock | ⬜ |
| shaman+warlock+druid | ⬜ |
| shaman+druid+druid | ⬜ |
| mage+mage+mage | ⬜ |
| mage+mage+warlock | ⬜ |
| mage+mage+druid | ⬜ |
| mage+warlock+warlock | ⬜ |
| mage+warlock+druid | ⬜ |
| mage+druid+druid | ⬜ |
| warlock+warlock+warlock | ⬜ |
| warlock+warlock+druid | ⬜ |
| warlock+druid+druid | ⬜ |
| druid+druid+druid | ⬜ |

## 2v2 спек-варианты — 1 (матч по точному составу врага)

Класс-уровень такой пары может быть уже ✅ — здесь отслеживаются спек-вариации, где тактика реально иная. Считаются отдельно, сверх 255 класс-ячеек.

| Enemy spec \ Our | RM | RP | Примечание |
|---|---|---|---|
| Rogue / Shadow Priest | ✅ | ✅ | Shadow≠Disc: оффенс-прийст (fear/silence/dispel), без хила, immobile |
